...
class PhotozGP:
    ...
    def updateHyperparamatersAndReturnMarglike_continuum(self, pars):
        """
        For optimizing Hyperparamaters with marglike as objective using scipy - continuum only.
        """
        V_C, alpha_C = pars
        self.kernel.var_C = 1*V_C
        self.kernel.alpha_C = 1*alpha_C
        self.KXX = self.kernel.K(self.X)
        self.A = self.KXX + np.diag(self.Yvar.flatten())
        sign, self.logdet = np.linalg.slogdet(self.A)
        self.logdet *= sign
    ...
    def updateHyperparamatersAndReturnMarglike_lines(self, pars):
        """
        For optimizing Hyperparamaters with marglike as objective using scipy - lines only.
        """
        V_L, alpha_L = pars
        self.kernel.var_L= 1*V_L
        self.kernel.alpha_L = 1*alpha_L
        self.KXX = self.kernel.K(self.X)
        self.A = self.KXX + np.diag(self.Yvar.flatten())
        sign, self.logdet = np.linalg.slogdet(self.A)
        self.logdet *= sign
